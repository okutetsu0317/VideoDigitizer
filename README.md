...
開発中の次期Macアプリ版では、最後に選択した動画への参照をアカウント別にMac内へ保存し、元のファイルが変更されていなければ次回起動時に再接続します。動画本体は複製もクラウド送信もしません。Googleログイン後にクラウド保存を明示的にONにした場合だけ、デジタイズデータを5分ごとに同期します。
...
次期Macアプリ版では任意のGoogleログインにより、同じMac内の自動保存を利用者ごとに分けて再開できます。クラウド保存をONにした場合だけ座標・点状態・較正等を同期し、動画、画像、ローカルパス、研究メタデータは送信しません。公開中のダウンロード版は、この機能を含む更新版の署名・公証が完了するまでは従来版のままです。
